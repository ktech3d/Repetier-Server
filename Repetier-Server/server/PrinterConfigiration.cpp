...
ExtruderConfiguration::ExtruderConfiguration(Poco::XML::Element *node) {
    ...
    for(int i=0;i<data->length();i++) {
        ...
        if(name == "eJerk") {
            eJerk = NumberParser::parseFloat(item->innerText());
        } else if(name == "maxSpeed")
            maxSpeed = NumberParser::parseFloat(item->innerText());
            ...
            if(e->hasAttribute("distance"))
                extrudeSpeed = NumberParser::parseFloat(e->getAttribute("distance"));
        } else if(name == "retract") {
            Element *e = (Element*)item;
            if(e->hasAttribute("speed"))
                retractSpeed = NumberParser::parseFloat(e->getAttribute("speed"));
            if(e->hasAttribute("distance"))
                retractSpeed = NumberParser::parseFloat(e->getAttribute("distance"));
...
void PrinterConfiguration::saveConfiguration() {
    ...
    setNodeText(getOrCreateElement("movement.xySpeed"),NumberFormatter::format(xySpeed));
    setNodeText(getOrCreateElement("movement.zSpeed"),NumberFormatter::format(zSpeed));
...
void PrinterConfiguration::fromJSON(json_spirit::mObject &obj) {
    try {
        ...
        xySpeed = movement["xySpeed"].get_real();
        zSpeed = movement["zSpeed"].get_real();
        xyJerk = movement["xyJerk"].get_real();
        zJerk = movement["zJerk"].get_real();
        printAcceleration = movement["xyPrintAcceleration"].get_real();
        travelAcceleration = movement["xyTravelAcceleration"].get_real();
